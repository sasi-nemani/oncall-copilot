...
- **The workflow is the point:** `terraform apply` → run the eval → **`terraform destroy`**. A self-hosted model has no rate limit but it *does* bill a GPU while it's up (~$0.75/hr), so the honest pattern is one command up, one command down — the box lived ~2 hours total (~$1–2), not 24/7. IAM least-privilege too: the VM's service account has **zero roles** (it only serves HTTP) and the endpoint firewall is locked to my IP.
...
- **The one bug that mattered:** the first VM booted but served *no* models. Cause: my startup script ran `ollama pull` immediately after `systemctl restart ollama`, but restart returns before the server is actually listening — the pull raced the socket and failed silently (and my retry loop didn't fail the script, so it exited "successfully" with nothing installed). Fix: poll `/api/tags` until the API answers *before* pulling, and log a loud WARNING if a model isn't present afterward. Classic distributed-systems lesson — "the service started" is not "the service is ready."
